...
| `tempo.observability.grafana.datasource` | `false` | Whether a Datasource managed by Grafana Operator should be created for Tempo. |
| `tempo.observability.metrics.serviceMonitors` | `false` | Whether ServiceMonitors managed by Prometheus Operator should be created for Tempo. |
| `tempo.observability.traces.jaegerEndpoint` | `""` | The Jaeger endpoint where to send traces. Only for TempoStack. |
| `tempo.observability.traces.sampling` | `""` | Sampling frequence for Tempo traces. Only for TempoStack. |
...
| `tempo.storage.backend.size` | `2Gi` | The size of the storage used by Tempo. |
